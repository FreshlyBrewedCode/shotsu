...
The system SHALL provide edit mode at `/me/sets/[id]?mode=edit` that renders the set editor for the set identified by `id`. The edit mode SHALL be activated by a `?mode=edit` search parameter on the same route as the viewer. If no set with that id exists in local state, the page SHALL display an error or redirect to the home page.

#### Scenario: Navigating to the editor for a valid set
- **WHEN** the user navigates to `/me/sets/[id]?mode=edit` for an existing set id
...
- **WHEN** the user navigates to `/me/sets/[id]?mode=edit` for an id that does not exist in local state
...
The system SHALL provide a "plus" button in the top navigation bar. Clicking it SHALL create a new set in local state and navigate the user to the new set's page at `/me/sets/[new-set-id]?mode=edit`.
...
- **AND** the user SHALL be navigated to `/me/sets/[new-set-id]?mode=edit`
