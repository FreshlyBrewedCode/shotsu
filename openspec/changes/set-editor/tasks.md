...
- [ ] 4.4 Implement `useReducer` with actions: `CREATE_SET`, `UPDATE_SET_TITLE`, `ADD_SECTION`, `REMOVE_SECTION`, `UPDATE_SECTION_LAYOUT`, `MOVE_SECTION`, `ADD_PHOTO`, `REMOVE_PHOTO`
...
## 5. Navigation & New Set Entry Point
...
- [ ] 6.4 Render all sections in order using the existing `Section` and `Photo` components
...
- [ ] 6.7 Wrap each Section and Photo with edit-mode interaction affordances (hover border, context menu trigger)
...
- [ ] 9.1 Verify IndexedDB save/load round-trip: create set, reload page, set is present
- [ ] 9.2 Verify ingest pipeline: select an image, confirm blob stored, catalog index updated, photo renders in editor
- [ ] 9.3 Verify EXIF extraction: select a photo with EXIF, confirm metadata in catalog index
- [ ] 9.4 Verify resize: select an image >2400px, confirm output dimensions are capped
- [ ] 9.5 Verify object URL cleanup: confirm no memory leak warnings on section/photo unmount
- [ ] 9.6 Verify the editor renders correctly on mobile viewport sizes
- [ ] 9.7 Verify context menu appears as bottom sheet on mobile and popover on desktop
- [ ] 9.8 Verify "Remove Section" is blocked when only one section remains
- [ ] 9.9 Verify "Move Section Up/Down" boundary conditions are no-ops
- [ ] 9.10 Run `npm run build` and resolve any TypeScript or lint errors
