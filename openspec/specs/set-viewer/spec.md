...
The system SHALL provide a read-only page at `/sets/[id]` that renders the set identified by `id` using the same visual components as the set editor, but without edit affordances. If no set with that id exists in local state, the page SHALL redirect to the home page.

#### Scenario: Navigating to the viewer for a valid set
- **WHEN** the user navigates to `/sets/[id]` for an existing set id
...
- **WHEN** the user navigates to `/sets/[id]` for an id that does not exist in local state
